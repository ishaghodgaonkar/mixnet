...
class mixBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, stride=stride, padding=kernel_size//2)
    ...
    def forward(self, input):
        ...
        # implementing skip connection, similar to RESnet, by preserving input information
...
class Retina(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        edge detection or sharpen kernel. Initialize weights as that kernel, and they will adjust from there.
        """
        weights = torch.tensor([[-1., -1., -1.],
                                [-1., 8., -1.],
                                [-1., -1., -1.]])

        # reshape and copy for 16 channel output
        ...
        """
        Expect to see similar receptive fields as ganglion cells (circular, activated in the center or in periphery). 
        """

        self.ganglion_conv2 = nn.Conv2d(32, out_channels, kernel_size=3, stride=1)

    def forward(self, input):

        x = self.bipolar_conv(input)
        x = self.ganglion_conv(x)
        x = self.ganglion_conv2(x)
...
def mixnetV1():
    model = nn.Sequential(
        OrderedDict([
            # Images are grey-scaled to result in one input channel
            ('Retina', Retina(1, 64)),
            ('V1', mixBlock(64, 128, kernel_size=5)),
            ('V2', mixBlock(128, 256)),
            ('V4', mixBlock(256, 512)),
